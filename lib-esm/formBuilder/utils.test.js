...
import { addCardObj, addSectionObj, checkForUnsupportedFeatures, DEFAULT_INPUT_NAME, excludeKeys, generateCategoryHash, generateElementComponentsFromSchemas as _generateElementComponentsFromSchemas, generateElementPropsFromSchemas as _generateElementPropsFromSchemas, generateSchemaFromElementProps as _generateSchemaFromElementProps, generateUiSchemaFromElementProps as _generateUiSchemaFromElementProps, getCardCategory as _getCardCategory, getNewElementDefaultDataOptions as _getNewElementDefaultDataOptions, getRandomId, parse as _parse, stringify as _stringify, subtractArray } from "./utils.js";
var schema = {
    type: 'object',
    properties: {
        obj1: {
...
var parse = _parse;
var getCardCategory = _getCardCategory;
var stringify = _stringify;
var generateElementPropsFromSchemas = _generateElementPropsFromSchemas;
var generateElementComponentsFromSchemas = _generateElementComponentsFromSchemas;
var generateSchemaFromElementProps = _generateSchemaFromElementProps;
var generateUiSchemaFromElementProps = _generateUiSchemaFromElementProps;
var getNewElementDefaultDataOptions = _getNewElementDefaultDataOptions;
function generateSchemaWithUnnamedProperties(amount) {
    var properties = __spreadArray([], Array.from(Array(10).keys()), true).reduce(function (acc, id) {
        var _a;
        return __assign(__assign({}, acc), (_a = {}, _a["".concat(DEFAULT_INPUT_NAME).concat(id + 1)] = {
            type: 'string'
        }, _a));
    }, {});
    return {
...
describe('getCardCategory', function () {
    it('returns the correct category for cards without ui refinements', function () {
        ...
        var categoryHash = generateCategoryHash(DEFAULT_FORM_INPUTS);
        expect(getCardCategory(card, categoryHash, DEFAULT_FORM_INPUTS)).toEqual('shortAnswer');
        card = {
            dataOptions: {
                type: 'integer'
            },
        ...
        expect(getCardCategory(card, categoryHash, DEFAULT_FORM_INPUTS)).toEqual('integer');
        card = {
            dataOptions: {
                type: 'number'
            },
        ...
        expect(getCardCategory(card, categoryHash, DEFAULT_FORM_INPUTS)).toEqual('number');
        card = {
            dataOptions: {
                type: 'boolean'
            },
        ...
        expect(getCardCategory(card, categoryHash, DEFAULT_FORM_INPUTS)).toEqual('checkbox');
        card = {
            dataOptions: {
                type: 'array'
            },
        ...
        expect(getCardCategory(card, categoryHash, DEFAULT_FORM_INPUTS)).toEqual('array');
        card = {
            dataOptions: {
                enum: [0, 1, 2, 3, 4, 5]
            },
        ...
        expect(getCardCategory(card, categoryHash, DEFAULT_FORM_INPUTS)).toEqual('dropdown');
    });
    it('returns the correct category for cards with ui refinements', function () {
        ...
        var categoryHash = generateCategoryHash(DEFAULT_FORM_INPUTS);
        expect(getCardCategory(card, categoryHash, DEFAULT_FORM_INPUTS)).toEqual('password');
        card = {
            dataOptions: {
                enum: [0, 1, 2, 3, 4, 5]
            },
        ...
        expect(getCardCategory(card, categoryHash, DEFAULT_FORM_INPUTS)).toEqual('radio');
        card = {
            dataOptions: {
                type: 'array'
            },
...
describe('checkForUnsupportedFeatures', function () {
    it('gives no warnings for various valid combinations', function () {
        ...
        var testUischema = {};
        expect(checkForUnsupportedFeatures(testSchema, testUischema, DEFAULT_FORM_INPUTS)).toEqual([]);
        testSchema = {
            type: 'object',
            properties: {
                obj1: {
    ...
    it('gives warnings for unknown features in schema', function () {
        ...
        expect(checkForUnsupportedFeatures(testSchema, testUischema, DEFAULT_FORM_INPUTS)).toEqual(['Unrecognized Object Property: erroneousKey']);
    });
    it('gives warnings for unknown features in ui schema', function () {
        ...
        expect(checkForUnsupportedFeatures(testSchema, testUischema, DEFAULT_FORM_INPUTS)).toEqual(['Unrecognized UI schema property: ui:error', 'UI Property: ui:error2 for obj1']);
    });
});
...
describe('generateSchemaFromElementProps', function () {
    var schemaProps = generateSchemaFromElementProps(elementPropArr, DEFAULT_FORM_INPUTS);
    ...
    it('throws an exception if propType is invalid', function () {
        expect(function () { return generateSchemaFromElementProps([{
                ...
                propType: 'foobar'
            }], DEFAULT_FORM_INPUTS); }).toThrow(new Error('Element that is neither card, section, nor ref'));
    });
...
describe('generateUiSchemaFromElementProps', function () {
    var uiSchema = generateUiSchemaFromElementProps(elementPropArr, DEFAULT_FORM_INPUTS);
...
describe('generateElementComponentsFromSchemas', function () {
    it('propagates mods to Section component', function () {
        var MockComponent = jest.fn(function () { return React.createElement("div", null); });
        var mods = {
            customFormInputs: {
                test: {
                    displayName: 'Test',
        ...
        var allFormInputs = __assign(__assign({}, DEFAULT_FORM_INPUTS), mods.customFormInputs);
        var categoryHash = generateCategoryHash(allFormInputs);
        var TestComponent = function () { return React.createElement(React.Fragment, null, generateElementComponentsFromSchemas({
            schemaData: {
                type: 'object',
                properties: {
                    section1: {
        ...
        mount(React.createElement(TestComponent, null), {
            attachTo: div
        });
...
describe('subtractArray', function () {
    it('returns the first array if the second array is undefined', function () {
        ...
        var actualResult = subtractArray(array1, array2);
...
describe('excludeKeys', function () {
    it('returns the given object excluding the given keys', function () {
        ...
        var actualResult = excludeKeys(obj, keys);
        expect(actualResult).toEqual(expectedResult);
    });
