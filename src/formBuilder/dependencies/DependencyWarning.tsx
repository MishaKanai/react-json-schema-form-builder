  ...
  if (parameters.enum && parameters.dependents && parameters.dependents.length && parameters.dependents[0].value) {
    ...
    return <React.Fragment>
        <p>
          ...
          values:{' '}
          <Tooltip type='help' text='Each possible value for a value-based dependency must be defined to work properly' />
        </p>

        <ul>
          {undefinedVals.map((val, index) => <li key={index}>{val}</li>)}
        </ul>
      </React.Fragment>;
  }
