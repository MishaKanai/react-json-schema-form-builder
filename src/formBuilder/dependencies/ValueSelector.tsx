...
export default function ValueSelector({
  ...
  if (possibility.value) {
    ...
    if (parentEnums) {
      ...
      if (enumType === 'string') return <CardSelector possibleChoices={parentEnums.map(val => `${val}`)} chosenChoices={possibility.value.enum} onChange={(chosenChoices: Array<string>) => onChange({
        ...possibility,
        value: {
          enum: chosenChoices
        }
      })} placeholder='Allowed value' path={path} />;
      if (enumType === 'number') return <CardSelector possibleChoices={parentEnums.map(val => `${val}`)} chosenChoices={possibility.value.enum} onChange={(chosenChoices: Array<string>) => onChange({
        ...possibility,
        value: {
    ...
    if (parentType === 'boolean') {
      return <FBCheckbox onChangeValue={() => {
    ...
    if (parentType === 'object') {
      const enumArr = (possibility.value.enum as Array<any>);
      return <div>
        {enumArr.map((combination, index) => <li key={`${elementId}_possibleValue${index}`}>
          {Object.keys(combination).map(key => {
            const val = combination[key];
            return <div key={key}>
              <h5>{key}:</h5>
              {{
                string: <Input value={val || ''} placeholder='String value' /* type='string' */ onChange={(ev: any) => {
                  const newVal = ev.target.value;
                  const oldCombo = possibility.value.enum[index];
                  onChange({
                    ...possibility,
                    value: {
                      enum: [...enumArr.slice(0, index), {
                        ...oldCombo,
                        [key]: newVal
                ...
                number: <Input value={val || ''} placeholder='Number value' type='number' onChange={(ev: any) => {
                ...
                array: <Input value={JSON.stringify(val) || ''} placeholder='Array in JSON' /* type='string' */ onChange={(ev: any) => {
                  let newVal = val;
                ...
                object: <Input value={JSON.stringify(val) || ''} placeholder='Object in JSON' /* type='string' */ onChange={(ev: any) => {
                  let newVal = val;
    ...
    return <CardEnumOptions initialValues={possibility.value.enum} onChange={(newEnum: Array<any>) => onChange({
      ...possibility,
      value: {
        enum: newEnum
      }
