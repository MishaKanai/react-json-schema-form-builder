...
export function defaultDataProps(category: string, allFormInputs: Record<string, FormInput>): Record<string, any> {
  return allFormInputs[category].defaultDataSchema;
}
export function defaultUiProps(category: string, allFormInputs: Record<string, FormInput>): Record<string, any> {
  return allFormInputs[category].defaultUiSchema;
}
export function categoryType(category: string, allFormInputs: Record<string, FormInput>): DataType {
  return allFormInputs[category].type;
}
export function getCardBody(category: string, allFormInputs: Record<string, FormInput>): CardBody {
  return allFormInputs[category] && allFormInputs[category].cardBody || (() => null);
}
export function categoryToNameMap(category: string, allFormInputs: Record<string, FormInput>): Record<string, string> {
...
export function generateCategoryHash(allFormInputs: Record<string, FormInput>): Record<string, string> {
  const categoryHash = {};
...
export function getCardCategory(cardProps: CardProps, categoryHash: Record<string, string>): string {
...
function checkObjectForUnsupportedFeatures(schema: Record<string, any>, uischema: Record<string, any>, supportedWidgets: Set<string>, supportedFields: Set<string>, supportedOptions: Set<string>) {
  ...
  if (schema.properties) Object.entries(schema.properties).forEach(([parameter, element]: [string, any]) => {
...
export function checkForUnsupportedFeatures(schema: Record<string, any>, uischema: Record<string, any>, allFormInputs: Record<string, FormInput>): string[] {
  // add each unsupported feature to this array
  const unsupportedFeatures = [];
  const widgets = [];
  const fields = [];
...
function generateDependencyElement(name: string, dataProps: any, uiProperties: any, requiredNames: any, definitionData?: Record<string, any>, definitionUi?: Record<string, any>, categoryHash?: Record<string, string>, useDefinitionDetails: boolean = true) // determines whether to use an element's definition details or not.
{
  let uiProps = {
    ...uiProperties
  };
...
export function generateElementPropsFromSchemas(parameters: {
  ...
  Object.entries(schema.properties).forEach(([parameter, element]) => {
    const newElement: any = {};
    let elementDetails = element && typeof element === 'object' ? (element as any & {
      $ref?: string;
    }) : {};
  ...
  return (cardPropList as Array<ElementProps>);
}
...
export function generateSchemaFromElementProps(elementArr: Array<ElementProps>): {
...
export function generateUiSchemaFromElementProps(elementArr: Array<ElementProps>, definitionUi: any): {
  definitions?: Record<string, any>;
  [key: string]: any;
} {
  if (!elementArr) return {};
...
export function getCardParameterInputComponentForType(category: string, allFormInputs: Record<string, FormInput>): ModalBody {
...
export function updateSchemas(elementArr: Array<ElementProps>, parameters: {
  schema: {
    definitions?: Record<string, any>;
    [key: string]: any;
  };
...
export function generateElementComponentsFromSchemas(parameters: {
  ...
  const elementList = elementPropArr.map((elementProp, index) => {
    ...
    if (elementProp.propType === 'card') {
      ...
      return <Card componentProps={Object.assign({
      ...
      }, elementPropArr[index].uiOptions, elementPropArr[index].dataOptions)} key={`${path}_${elementPropArr[index].name}`} TypeSpecificParameters={TypeSpecificParameters} onChange={(newCardObj: Record<string, any>) => {
        const newElementObjArr = generateElementPropsFromSchemas({
          schema,
          uischema,
          definitionData,
      ...
      }} cardOpen={expanded} setCardOpen={(newState: boolean) => setCardOpenArray([...cardOpenArray.slice(0, index), newState, ...cardOpenArray.slice(index + 1)])} allFormInputs={allFormInputs} mods={mods} />;
    } else if (elementProp.propType === 'section') {
      // create a section with the appropriate schemas here
      return <Section schema={elementProp.schema} uischema={elementProp.uischema} onChange={(newSchema: Record<string, any>, newUiSchema: Record<string, any>, newRef?: string) => {
        const newElementObjArr = generateElementPropsFromSchemas({
          schema,
          uischema,
          definitionData,
      ...
      }} onDependentsChange={(newDependents: Array<{
        ...
        setCardOpenArray([...cardOpenArray, false]);
      }} cardOpen={expanded} setCardOpen={(newState: boolean) => setCardOpenArray([...cardOpenArray.slice(0, index), newState, ...cardOpenArray.slice(index + 1)])} allFormInputs={allFormInputs} categoryHash={categoryHash} mods={mods} />;
    } else {
      return <div>
...
export function onDragEnd(result: any, details: {
  schema: Record<string, any>;
  uischema: Record<string, any>;
  onChange: (arg0: Record<string, any>, arg1: Record<string, any>) => any;
  definitionData?: Record<string, any>;
...
function propagateElementChange(elementArr: Array<ElementProps>, definitionData: Record<string, any>, definitionUi: Record<string, any>, categoryHash: Record<string, string>) {
...
export function propagateDefinitionChanges(schema: Record<string, any>, uischema: Record<string, any>, onChange: (arg0: Record<string, any>, arg1: Record<string, any>) => void, categoryHash: Record<string, string>) {
...
export function subtractArray(array1: Array<string>, array2?: Array<string>): Array<string> {
...
export function excludeKeys(obj: Record<string, any>, keys: string[] | null | undefined): Record<string, any> {
  if (!keys) return {
    ...obj
  };
  ...
  return Object.keys(obj).reduce((acc, curr) => keysHash[curr] ? acc : {
    ...acc,
    [curr]: obj[curr]
  }, {});
}
export function getNewElementDefaultDataOptions(i: number, mods?: Mods): DataOptions {
  if (mods && mods.newElementDefaultDataOptions !== undefined) {
    ...
    return {
      ...mods.newElementDefaultDataOptions,
      ...{
        title: title
      }
    };
