...
const props = {
  ...
  TypeSpecificParameters: ({
    parameters,
    onChange
  }) => <input className='inputVal' value={parameters.inputVal || ''} onChange={val => {
    onChange({ ...parameters,
...
describe('CardModal', () => {
  ...
  it('calls the close function on cancel', () => {
    const wrapper = mount(<CardModal {...props} />);
    expect(wrapper.exists('div[data-test="card-modal"]')).toBeTruthy();
    const cancelButton = wrapper.find('div[data-test="card-modal"]').find('.btn-secondary').first();
  ...
  it('calls the change and close functions on save', () => {
    const wrapper = mount(<CardModal {...props} />);
    expect(wrapper.exists('div[data-test="card-modal"]')).toBeTruthy();
    const saveButton = wrapper.find('div[data-test="card-modal"]').find('.btn-primary').first();
  ...
  it('calls the onChange with a new minimum length when minimum length is altered', () => {
    const wrapper = mount(<CardModal {...props} />);
    expect(wrapper.exists('div[data-test="card-modal"]')).toBeTruthy();
    const specificField = wrapper.find('div[data-test="card-modal"]').first().find('.inputVal').first();
